...
public class DbManager extends SQLiteOpenHelper{
    ...
    public Cursor getSelectedBook(String id){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "select * from "+TABLENAME +" where id=4";
        //String query = "select * from users where username = 'bhawana'";

        Cursor crs = db.rawQuery(query,null);
        return crs;
    }
    public Cursor updateBookOnClick(String id, String title, String description, String author, String price){ //we can change string with boolean if returning true or false
        //return "ok";
        SQLiteDatabase db = this.getWritableDatabase();
